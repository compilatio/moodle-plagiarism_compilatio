...
//Connection test : file_get_contents will return false if an error occurs. Connection is active in any other case.
if(isset($plagiarismsettings["compilatio_api"]))
	$connectionSuccess = @file_get_contents($plagiarismsettings["compilatio_api"]) !== false;
else
	$connectionSuccess = @file_get_contents("http://service.compilatio.net/webservices/CompilatioUserClient2.wsdl") !== false;
...
if(isset($plagiarismsettings["compilatio_password"], $plagiarismsettings["compilatio_api"]))
	$APIKeySuccess = testConnection($plagiarismsettings["compilatio_password"], $plagiarismsettings["compilatio_api"]);
else
	$APIKeySuccess = false;
...
else
{
	...
	{
		if($cronFrequency->value > 15 || $lastCron->value <= strtotime("-1 hour"))
		{//Warning
