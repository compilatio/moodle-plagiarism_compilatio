...
class migration extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        $params = [
            ...
            CURLOPT_HTTPHEADER => array('X-Auth-Token: ' . "6149cad18d500f041d0b38aeca1aedabaadbf35d", 'Content-Type: application/json'),
            CURLOPT_URL => "https://app.compilatio.net/api/private/documents/list?limit=1000&page=50&sort[metadata.indexed]=1&projection=".json_encode(["old_prod_id" => true]),
        ...
        foreach ($response->data->documents as $doc) {
            if (isset($doc->old_prod_id)) {
                ...
                $item->identifier = "999";
                $item->filename = "999";
                $item->externalid = $doc->old_prod_id;
                $DB->insert_record('plagiarism_compilatio_files', $item);
            }
        }*/
        ...
        for ($i = 0; $i < 40; $i++) {
            $v4files = $DB->get_records_select("plagiarism_compilatio_files", "CHAR_LENGTH(externalid) = 32 && migrationstatus IS NULL", null, '', '*', 0, 25);
            ...
            $apiconfig = $DB->get_record('plagiarism_compilatio_apicon', array('id' => get_config('plagiarism_compilatio', 'apiconfigid')));
            ...
            mtrace(var_export($result,true));

            foreach ($result->data->responses as $response) {

                if (isset($response->data->document)) {
                    $file = $DB->get_record("plagiarism_compilatio_files", array("externalid" => $response->data->document->old_prod_id));
                    if (!empty($file)) {
