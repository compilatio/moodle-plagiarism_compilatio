...
class CompilatioDocumentFrame {
            ...
            if ($canviewreport) {
                ...
                $documentframe =
                    "<a href='{$href}' target='_blank' class='cmp-btn cmp-btn-doc cmp-btn-primary'>"
                        . CompilatioIcons::report() . htmlspecialchars(get_string('report', 'core')) .
                    "</a>";
            }
    ...
    public static function get_score($cmpfile, $config, $isteacher, $nowrap = false) {
        ...
        $html = '<span title="' . htmlspecialchars($title) . '" class="cmp-similarity cmp-color-' . $color . ' align-middle">
