...
define('COMPILATIO_MAX_SUBMISSION_ATTEMPTS', 6); //maximum number of times to try and send a submission to Compilatio
define('COMPILATIO_MAX_SUBMISSION_DELAY', 60); //maximum time to wait between submissions (defined in minutes)
define('COMPILATIO_SUBMISSION_DELAY', 15); //initial wait time - this is doubled each time a check is made until the max_submission_delay is met
...
define('COMPILATIO_STATUS_DELAY', 30); //initial wait time - this is doubled each time a check is made until the max_status_delay is met.
...
class plagiarism_plugin_compilatio extends plagiarism_plugin {
    ...
    public function get_links($linkarray) {
        ...
        if ($results['statuscode'] == 'Analyzed') {
            ...
            $output .= '</span>';
        } elseif ($results['statuscode'] == COMPILATIO_STATUSCODE_ACCEPTED) {
            if (has_capability('moodle/plagiarism_compilatio:triggeranalysis', $modulecontext)) {
        ...
        } elseif ($results['statuscode'] == COMPILATIO_STATUSCODE_ANALYSING) {
            $output .= '<span class="plagiarismreport">'.
                '<img src="'.$OUTPUT->pix_url('processing', 'plagiarism_compilatio') .
                '" alt="'.get_string('processing', 'plagiarism_compilatio').'" '.
                '" title="'.get_string('processing', 'plagiarism_compilatio').'" />'.
                '</span>';
        } elseif ($results['statuscode'] == COMPILATIO_STATUSCODE_UNSUPPORTED) {
            $output .= '<span class="plagiarismreport">'.
                       '<img src="'.$OUTPUT->pix_url('warning', 'plagiarism_compilatio') .
                        ...
                        '</span>';
        } elseif ($results['statuscode'] == COMPILATIO_STATUSCODE_TOO_LARGE) {
            $output .= '<span class="plagiarismreport">'.
                       '<img src="'.$OUTPUT->pix_url('warning', 'plagiarism_compilatio') .
    ...
    function get_file_results($cmid, $userid, stored_file $file) {
        ...
        if ($file->get_filename() !== $plagiarismfile->filename) {
            $previouslysubmitted = '('.get_string('previouslysubmitted','plagiarism_compilatio').': '.$plagiarismfile->filename.')';
        }
    ...
    */
    ...
    public function get_form_elements_module($mform, $context) {
        ...
        if ($mform->elementExists('seuil_faible')) { //Hack to prevent this from showing on custom compilatioassignment type.
            return;
        }
    ...
    function compilatio_send_student_email($plagiarism_file) {
        global $DB, $CFG;
...
function compilatio_get_form_elements($mform, $defaults=false) {
    ...
    if (!$defaults) { //only show this inside a module page - not on default settings pages.
        $mform->addElement('date_time_selector', 'compilatio_timeanalyse', get_string('analysisdate', 'compilatioassignment'), array('optional'=>false));
...
function compilatio_check_attempt_timeout($plagiarism_file) {
    ...
    $maxattempts = 4;
    if ($plagiarism_file->statuscode == 'pending') {
        $submissiondelay = COMPILATIO_SUBMISSION_DELAY; //initial wait time - doubled each time a check is made until the max delay is met.
        $maxsubmissiondelay = COMPILATIO_MAX_SUBMISSION_DELAY; //maximum time to wait between submissions
        $maxattempts = COMPILATIO_MAX_SUBMISSION_ATTEMPTS; //maximum number of times to try and send a submission.
    } else if ($plagiarism_file->statuscode ==COMPILATIO_STATUSCODE_ACCEPTED) {
        $submissiondelay = COMPILATIO_STATUS_DELAY; //initial wait time - this is doubled each time a check is made until the max delay is met.
        $maxsubmissiondelay = COMPILATIO_MAX_STATUS_DELAY; //maximum time to wait between checks
        $maxattempts = COMPILATIO_MAX_STATUS_ATTEMPTS; //maximum number of times to try and send a submission.
    }
...
function compilatio_get_scores($plagiarismsettings) {
    global $DB;
    //$allowedstatus = array(COMPILATIO_STATUSCODE_PROCESSED,
    //                       COMPILATIO_STATUSCODE_NOT_FOUND,
    //                       COMPILATIO_STATUSCODE_BAD_REQUEST);
